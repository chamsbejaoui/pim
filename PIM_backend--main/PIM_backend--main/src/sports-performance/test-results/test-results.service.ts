...
export class TestResultsService {
    ...
    async update(id: string, dto: UpdateTestResultDto): Promise<TestResult> {
        ...
        // Si la valeur brute change, recalculer le score via ScoringService
        if (dto.rawValue !== undefined) {
            const existing = await this.testResultModel
                .findById(id)
                .select('testTypeId')
                .exec();
            ...
            updateData.normalizedScore = await this.scoringService.calculateNormalizedScore(
                dto.rawValue,
                existing.testTypeId.toString(),
            );
        }
