...
export class PlayerService {
    ...
    async findAll(): Promise<Player[]> {
        return this.playerModel.find({ $or: [{ status: 'active' }, { status: { $exists: false } }] }).exec();
    }
    ...
    async count(): Promise<number> {
        return this.playerModel.countDocuments({ $or: [{ status: 'active' }, { status: { $exists: false } }] }).exec();
    }
