...
export class AuthService {
  ...
  async verifySensitiveAction(
    userId: string,
    input: SensitiveVerificationInput,
  ): Promise<boolean> {
    ...
    // ODIN backend currently validates sensitive actions via password re-auth.
    if (!input?.password || typeof input.password !== 'string') {
      return false;
    }

    return this.userService.validatePassword(user, input.password);
  }
}
