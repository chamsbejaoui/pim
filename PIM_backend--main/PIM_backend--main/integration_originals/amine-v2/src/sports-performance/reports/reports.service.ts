...
export class ReportsService {
    ...
    /**
     * Récupère la progression historique d'un joueur (Tests vs Matchs)
     */
    async getPlayerProgression(playerId: string): Promise<any> {
        if (!Types.ObjectId.isValid(playerId)) {
            throw new BadRequestException(`Invalid Player ID: ${playerId}`);
        }

        // 1. Récupérer toutes les participations du joueur
        const participations = await this.eventPlayerModel
            .find({ playerId: new Types.ObjectId(playerId) })
            .populate('eventId')
            .exec();

        // 2. Récupérer tous les rapports individuels du joueur
        const reports = await this.playerReportModel
            .find({
                eventPlayerId: { $in: participations.map(p => p._id) }
            })
            .sort({ generatedAt: 1 }) // Chronologique
            .exec();

        // 3. Organiser les données par type d'événement
        const testsProgression = reports.map(r => ({
            date: r.generatedAt,
            score: r.overallScore,
            type: 'Test'
        }));

        const matchProgression = participations
            .filter(p => (p.eventId as any)?.type === 'match' && p.aiAnalysis?.metrics)
            .map(p => ({
                date: (p.eventId as any).date,
                score: p.aiAnalysis?.metrics?.speed ?? 0, // Utilisons la vitesse comme métrrique par défaut pour match
                distance: p.aiAnalysis?.metrics?.distance ?? 0,
                type: 'Match'
            }))
        ...
        return {
            playerId,
            tests: testsProgression,
            matches: matchProgression,
            metricsAvailable: ['overallScore', 'speed', 'endurance', 'distance']
        };
    }
}
